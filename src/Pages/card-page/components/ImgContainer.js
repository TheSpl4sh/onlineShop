...
function ImgContainer({ logo_img, chips, isFavorite, onToggleFavorite }) {
  const newColor = "#FF6915";
  ...
  return (
    <div className="img-container">
      ...
      {isFavorite ? (
        <BsHeartFill
          className="card-page-heart-icon"
          onClick={onToggleFavorite}
          fill={newColor}
        />
      ) : (
        <BsHeart className="card-page-heart-icon" onClick={onToggleFavorite} />
      )}
    </div>
  );
}
